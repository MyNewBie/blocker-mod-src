...
//	dbg_msg("DENNIS", "sending serverinfo %i with %i clients and %i dummies (max: %i)", InfoType, ClientCount, DummyCount, MaxClients);
	...
	str_format(aBuf, sizeof(aBuf), "%d", MaxClients); p.AddString(aBuf, -1); // max clients
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(pThis->m_aClients[i].m_State != CClient::STATE_EMPTY)
		{
			...
			if(pThis->m_aClients[i].m_State == CClient::STATE_INGAME || pThis->m_aClients[i].m_State == CClient::STATE_DUMMY)
			{
				...
				str_format(aBuf, sizeof(aBuf), "id=%d addr=%s name='%s' score=%d client=%d secure=%s %s", i, pThis->m_aClients[i].m_State == CClient::STATE_DUMMY?"Dummy":aAddrStr,
